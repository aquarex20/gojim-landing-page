...
├── updates.html        # What's New (reads updates.json)
├── updates.json        # Generated by Notion sync (committed by CI)
├── styles.css          # CSS styles and animations
├── script.js           # JavaScript for interactivity
├── scripts/
│   └── sync-notion-data.js
├── package.json        # Dev/sync only (@notionhq/client)
└── README.md           # This file
```
...
7. **What's New** (`updates.html`): Changelog table fed by `updates.json` (synced from Notion)

## Notion → `updates.json` (GitHub Actions)

- Workflow: `.github/workflows/sync-notion-data.yml` — runs **daily** at 05:00 UTC and supports **Run workflow** manually.
- Add repo secrets: `NOTION_TOKEN`, `NOTION_DATABASE_ID` (Notion **database** UUID; share the DB with your integration).
- Expected Notion columns (matched **case-insensitively**): `Status`, `Changes`, `Time`, `Version`.
- Local sync: copy `.env.example` → `.env`, then `npm install` and `npm run sync:notion`.
- **Zero rows** from Notion: the script does not overwrite `updates.json`, so nothing new is committed.
- Deploy on **Render** (or any static host): push includes `updates.json`; no GitHub Pages required.
