...
async function syncNotionToJson() {
  ...
  if (allPages.length === 0) {
    ...
    await appendStepSummary("**Result:** 0 rows returned — `updates.json` was not modified (no commit).");
  ...
  await appendStepSummary(`**Wrote:** \`${meaningful.length}\` row(s) to \`updates.json\`.`);
}
